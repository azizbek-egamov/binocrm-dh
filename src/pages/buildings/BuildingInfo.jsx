...
import { AdaptivePageHeader, useAdaptiveHeader } from '../../components/AdaptiveHeader';
...
const BuildingInfo = () => {
    ...
    const loadHomes = async () => {
        setLoading(true);
        try {
            const data = await getHomes({ building: selectedBuildingId, page_size: 1000 });
        ...
        Object.keys(structure).forEach(padez => {
            Object.keys(structure[padez]).forEach(floor => {
                structure[padez][floor].sort((a, b) => {
                    const numA = parseInt(String(a.number || a.home_number).replace(/\D/g, '')) || 0;
                    const numB = parseInt(String(b.number || b.home_number).replace(/\D/g, '')) || 0;
                    return numA - numB;
                });
            });
        });
    ...
    const stats = useMemo(() => {
        ...
        const inactive = homes.filter(h => h.status === 'INACTIVE' || h.status === 'UNAVAILABLE').length;
    ...
    const HomeTooltip = ({ home, position }) => {
        ...
        return createPortal(
            <div className="home-info-tooltip" style={{ top: position.top, left: position.left }}>
                ...
                <div className="tooltip-body">
                    <div className="tooltip-row">
                        ...
                        <div className="tooltip-value">{home.padez || home.padez_number || 1}</div>
                    </div>
                    <div className="tooltip-row">
    ...
    return (
        <div className="contract-create-page building-info-page">
                        ...
                        <div className="budget-stats-row">
                            <div className="budget-mini-stats">
                                <div className="stat-item">
                                    ...
                                    <span className="stat-value">{formatNumber(selectedBuilding.budget)} so'm</span>
                                </div>
                                <div className="stat-divider"></div>
                                <div className="stat-item">
                                    <span className="stat-label">Sarflandi:</span>
                                    <span className="stat-value spent">{formatNumber(selectedBuilding.spent_amount)} so'm</span>
                                </div>
                                <div className="stat-divider"></div>
                                <div className="stat-item">
                                    <span className="stat-label">Qoldi:</span>
                                    <span className={`stat-value remaining ${selectedBuilding.remaining_budget < 0 ? 'text-danger' : 'text-success'}`}>
                    ...
                    <div className="section-header">
                        ...
                        {homes.length > 0 && (
                            <div className="home-stats">
                                <span className="stat available">Sotuvda: {stats.available}</span>
            ...
            {/* Home Details Modal */}
            {showDetailsModal && selectedHome && createPortal(
                <div className="modal-overlay" onClick={closeModal}>
                    <div className="modal-content modal-lg home-details-modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <div className="modal-title">
                                ...
                                <div>
                                    <h3>Uy #{selectedHome.number || selectedHome.home_number}</h3>
                                    <span className={`status-badge ${getStatusClass(selectedHome)}`}>
                        ...
                        <div className="modal-body">
                            <div className="home-summary">
                                <div className="summary-item">
                                    ...
                                    <span className="value">{selectedHome.padez || selectedHome.padez_number || 1}</span>
                                </div>
                                <div className="summary-item">
                                    <span className="label">Qavat</span>
                                    <span className="value">{selectedHome.floor || selectedHome.home_floor}</span>
                                </div>
                                <div className="summary-item">
                                ...
                                <div className="summary-item">
                                    ...
                                    <span className="value">{formatNumber(selectedHome.price)} so'm</span>
                                </div>
                                <div className="summary-item">
                                    <span className="label">Umumiy narx</span>
                                    <span className="value highlight">{formatNumber(selectedHome.price * selectedHome.square_meter)} so'm</span>
                                </div>
                            </div>
                            ...
                            {selectedHome.status === 'SOLD' && (
                                <div className="contract-preview-section">
                                    ...
                                    {contractLoading ? (
                                        ...
                                        <div className="contract-info-card">
                                            <div className="info-row">
                                                <div className="info-item">
                                                    ...
                                                    <span className="value">{contractData.client_name || contractData.client?.full_name}</span>
                                                </div>
                                                <div className="info-item">
                                                    <span className="label">Telefon:</span>
                                                    <span className="value">{contractData.client?.phone || 'Mavjud emas'}</span>
                                                </div>
                                            </div>
                                            <div className="info-row">
                                                <div className="info-item">
                                                    <span className="label">Shartnoma:</span>
                                                    <span className="value">#{contractData.contract_number}</span>
                                                </div>
                                                <div className="info-item">
                                                    <span className="label">Sana:</span>
                                                    <span className="value">
                                                        {(() => {
                                                            const dateStr = contractData.contract_date || contractData.created_at;
                                        ...
                                        <div className="preview-error">Shartnoma ma'lumotlarini yuklab bo'lmadi</div>
                                    )}
                                </div>
                            )}
                                ...
                                <div className="images-section">
                                    ...
                                    <div className="images-row">
                                        {selectedHome.floor_plan && (
                                            <div className="image-card" onClick={() => setPreviewImage(selectedHome.floor_plan)}>
                                                <img src={selectedHome.floor_plan || "/placeholder.svg"} alt="Loyiha rasmi" />
                                                <span>Loyiha rasmi</span>
                                            </div>
                                        )}
                                        {selectedHome.cadastral_image && (
                                            <div className="image-card" onClick={() => setPreviewImage(selectedHome.cadastral_image)}>
                                                <img src={selectedHome.cadastral_image || "/placeholder.svg"} alt="Kadastr rasmi" />
                        ...
                        <div className="modal-actions">
                            <button className="btn-secondary" onClick={closeModal}>Yopish</button>
                        </div>
                    </div>
                </div>,
                document.body
