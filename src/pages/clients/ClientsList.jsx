...
const ClientsList = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast.success(force ? "Mijoz va shartnomalari o'chirildi" : "Mijoz o'chirildi");
            closeModal();
            loadClients();
        } catch (error) {
            if (error.response?.status === 409 && error.response?.data?.error === 'has_contracts') {
                ...
                <div className={`modal-overlay ${modalClosing ? 'closing' : ''}`} onClick={closeModal}>
                    <div className={`modal-content ${modalClosing ? 'closing' : ''}`} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body" style={{ textAlign: 'center', padding: '32px 24px' }}>
                            <div className="modal-icon danger" style={{ margin: '0 auto 20px', width: '64px', height: '64px', borderRadius: '50%', background: deleteWarning ? 'rgba(245,158,11,0.15)' : 'rgba(239, 68, 68, 0.1)', color: deleteWarning ? '#f59e0b' : '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <TrashIcon />
                            </div>
                            ...
                            {deleteWarning ? (
                                <div style={{
                                    ...
                                    borderRadius: '10px', padding: '14px', marginTop: '12px', textAlign: 'left'
                                }}>
                                    <p style={{ fontSize: '13px', fontWeight: 700, color: '#f59e0b', marginBottom: '8px' }}>
                                        ⚠️ {deleteWarning.message}
                                    </p>
                                    {deleteWarning.contracts.map(c => (
                                        <p key={c.id} style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                                            📄 #{c.contract_number} — {c.building_name}, {c.home_number}-xona
                                        </p>
                                    ))}
                        ...
                        <div className="modal-actions">
                            ...
                            <button className="btn-danger" onClick={handleDelete} disabled={saving}>
                                {saving ? 'O\'chirilmoqda...' : deleteWarning ? 'Hammasi bilan o\'chirish' : 'O\'chirish'}
                            </button>
                        </div>
                    </div>
                </div>,
