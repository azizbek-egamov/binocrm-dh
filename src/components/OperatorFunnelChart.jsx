...
export const OperatorFunnel = ({ operators, operatorFunnel, stages, themeColors }) => {
    const [selectedIdx, setSelectedIdx] = useState(null);

    const mainItems = useMemo(() => {
        const sorted = [...(operators || [])].sort((a, b) => (b.count || 0) - (a.count || 0));
    ...
    const detailItems = useMemo(() => {
        ...
        const d = operatorFunnel.find(f =>
            f.name.toLowerCase().trim() === (op.name || op.operator_name || "").toLowerCase().trim()
        );
        if (!d) return [];
        return stages
        ...
        <div>
            <div style={{
                ...
                <div style={{
                    ...
                    <AmFunnel
                        items={mainItems.map(item => ({ ...item, name: item.operator_name || item.name }))}
